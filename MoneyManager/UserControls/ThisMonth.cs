...
{
    public partial class ThisMonth : UserControl
    {
        MoneyManagerContext context = new MoneyManagerContext();
        Account acc;
        Wallet wallet;
        public ThisMonth(Account data, Wallet wallet)
        {
            InitializeComponent();
            acc = data;
            this.wallet = wallet;
        ...
        private void ThisMonth_Load(object sender, EventArgs e)
        {
            var list = wd.GetTransactionsInCurrentMonth(acc.AccountId, wallet.WalletId);
            ...
            foreach (var dateOnly in uniqueDates)
            {
                ...
                foreach (var trans2 in trans1)
                {
                    ...
                    picDel.Click += (sender, e) =>
                    {
                        ...
                        if (trans2.TransDate.HasValue && (now - trans2.TransDate.Value).TotalMinutes <= 1)
                        {
                        ...
                        {
                            MessageBox.Show("Chỉ có thể xóa trong vòng 1 phút sau khi tạo giao dịch!");
                        }
